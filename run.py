    ...
    if(len(response['services']) >= 1):
        ...
        for service in response['services']:
            ...
            if(service['tenant'] == answers['tenant']):
                set_state_array.append({'id': service['id'], 'state': 'waiting-deployment', 'protocol': service['protocol'],
                                     'tenant': service['tenant'], 'integration_environment': service['integration_environment']})
                for mock_deployer in pull_push_instances:
                    if mock_deployer['protocol'] == service['protocol'] and mock_deployer['integration_environment'] == service['integration_environment']:
                        print(json.dumps({'id': service['id'], 'state': 'deployed' if answers['service_state'] == 'deployed' else 'error', 'status_code': 200 if answers['service_state'] ==
                              'deployed' else 500, 'error_description': None if answers['service_state'] == 'deployed' else 'Mock Error', 'agent_id': mock_deployer['agent_id']}).encode('utf-8'))
                        ams_responses.append({'message': {'attributes': {'key': 'value'}, 'data': base64.b64encode(json.dumps({'id': service['id'], 'state': 'deployed' if answers['service_state'] == 'deployed' else 'error', 'status_code': 200 if answers[
                                            'service_state'] == 'deployed' else 500, 'error_description': None if answers['service_state'] == 'deployed' else 'Mock Error', 'agent_id': mock_deployer['agent_id']}).encode('utf-8')).decode('utf-8'), 'messageId': "12312323"}, "subscription": "projects/myproject/subscriptions/mysubscription"})
        if(len(set_state_array) >= 1):
            print(set_state_array)
            requests.put('http://127.0.0.1:5000/agent/set_services_state',
                         json=set_state_array, headers={'x-api-key': 'banana'})
            requests.post('http://127.0.0.1:5000/ams/ingest', json={'messages': ams_responses}, headers={
                          'Authorization': 'banana', 'content-type': 'application/json'})
